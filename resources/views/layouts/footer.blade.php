 <!-- Footer Start -->
 <div class="meeta-footer-5" style="background-image: url(assets/images/bg/footer-5-bg.jpg);">

    <!-- Footer Widget Start -->
    <div class="footer-widget text-center">
        <div class="container">

            <!-- Footer Logo Start -->
            <div class="footer-logo">
                <a href="index.html"><img src="{{asset ('assets/images/footer-logo-1.png') }}" alt="Logo"></a>
            </div>
            <!-- Footer Logo End -->

            <div class="footer-menu">
                <ul>
                    <li><a href="{{ route('welcome') }}">Home</a></li>
                    <li><a href="about.html">About Us</a></li>
                    <li><a href="speaker-single.html">Speaker</a></li>
                    <li><a href="event-single.html">Sponsor</a></li>
                    <li><a href="blog.html">Blog </a></li>
                    <li><a href="contact.html"> Contact</a></li>
                </ul>
            </div>

            <!-- Footer widget Social Start -->
            <div class="footer-widget-social">
                <a href="https://web.facebook.com/bess.yassine06" target={"_blank"}><i class="fab fa-facebook-f"></i></a>
                <a href="#"target={"_blank"}><i class="fab fa-github"></i></a>
                <a href="#" target={"_blank"}><i class="fab fa-linkedin-in"></i></a>
                <a href="#" target={"_blank"}><i class="fab fa-instagram"></i></a>
            </div>
            <!-- Footer widget Social End -->

            <!-- Footer Copyright Start -->
            <div class="footer-copyright">
                <p>2022 Copyright Meeta Designed by Pixelcurve. All Rights Reserved</p>
            </div>
            <!-- Footer Copyright End -->

        </div>
    </div>
    <!-- Footer Widget End -->

</div>
<!-- Footer End -->
